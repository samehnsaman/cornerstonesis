...
class RegistrationTest extends TestCase { use RefreshDatabase;
 public function test_student_can_register_only_once():void{$this->seed();$this->actingAs(User::first());$service=app(RegistrationService::class);$service->register(TermEnrollment::first(),Section::first());$this->assertDatabaseCount('registrations',1);$this->expectException(\Illuminate\Database\UniqueConstraintViolationException::class);$service->register(TermEnrollment::first(),Section::first());}
